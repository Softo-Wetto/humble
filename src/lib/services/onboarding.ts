...
export const onboardingSchema = z.object({
  ...
  photoCount: z.number().int().min(1).max(6),
}).refine((value) => value.minimumAge <= value.maximumAge, { path: ["maximumAge"], message: "Maximum age must be at least the minimum age." });
